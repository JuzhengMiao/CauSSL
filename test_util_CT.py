...
def test_all_case_withf(net, image_list, num_classes, patch_size=(112, 112, 80), stride_xy=18, stride_z=4, save_result=True, test_save_path=None, preproc_fn=None):
    total_metric = 0.0
    for image_path in tqdm(image_list):
        id = image_path.split('/')[-1][-7:-3]
        h5f = h5py.File(image_path.split('image')[0] + '/' + id + '.h5', 'r')
        ...
        if preproc_fn is not None:
            image = preproc_fn(image)
        prediction, score_map = test_single_case_withf(net, image, stride_xy, stride_z, patch_size, num_classes=num_classes)
...
def test_all_case_URPC(net, image_list, num_classes, patch_size=(112, 112, 80), stride_xy=18, stride_z=4, save_result=True, test_save_path=None, preproc_fn=None):
    total_metric = 0.0
    for image_path in tqdm(image_list):
        id = image_path.split('/')[-1][-7:-3]
        h5f = h5py.File(image_path.split('image')[0] + '/' + id + '.h5', 'r')
        ...
        if preproc_fn is not None:
            image = preproc_fn(image)
        prediction, score_map = test_single_case_URPC(net, image, stride_xy, stride_z, patch_size, num_classes=num_classes)
...
def test_single_case_withf(net, image, stride_xy, stride_z, patch_size, num_classes=1):
    w, h, d = image.shape
...
def test_single_case_URPC(net, image, stride_xy, stride_z, patch_size, num_classes=1):
    w, h, d = image.shape
    ...
    for x in range(0, sx):
        ...
        for y in range(0, sy):
            ...
            for z in range(0, sz):
                ...
                test_patch = np.expand_dims(np.expand_dims(test_patch,axis=0),axis=0).astype(np.float32)
                test_patch = torch.from_numpy(test_patch).cuda()
                y_main, _, _, _ = net(test_patch)   # 1
                y = F.softmax(y_main, dim=1)
                y = y.cpu().data.numpy()
                y = y[0,:,:,:,:]
                score_map[:, xs:xs+patch_size[0], ys:ys+patch_size[1], zs:zs+patch_size[2]] \
                  = score_map[:, xs:xs+patch_size[0], ys:ys+patch_size[1], zs:zs+patch_size[2]] + y
